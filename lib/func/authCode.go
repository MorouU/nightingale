...
// get auth code
func GetAuthCode(width,height int,text string)(b []byte){
	textLen := len(text)
...
func loadFontFace(points float64) font.Face {
	...
	b,_ := ioutil.ReadFile(appConfig.AuthCodePath)
...
func CmpAuthCodeTimeOut(before int64,lifeTime int) bool {
	return GetTimeUnix(false) < before + int64(lifeTime)
}
